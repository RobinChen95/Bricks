DATAS SEGMENT 
	...
	xx dw ?;XX运动向量 
    yy dw ?;YY运动向量 
    ballxx dw 140;记录球的左上角的x轴坐标 
    ballyy dw 183;记录球的左上角的y轴坐标 
...
;------------------------------------------------------------------------- 
;处理小球运动的函数 
move_handle2 proc near 
    push ax 
    push bx 
    push cx 
    push dx 
    mov color,00 
    call setball2;擦除并重新画小球2   
    call positions2
    cmp ballyy,185;如果小球落地，则生命减1
    jl notoutball
    push bx
    xor bx,bx
    mov bx,462h
...
judges2 proc near 
    ...
    mov ax,ballxx
    mov bx,ballyy
    sub ax,xx;ax中是球的上个x的坐标
    sub bx,yy;bx中是球的上个y的坐标
    mov startsy,21
    mov cx,27	
    hit_lie: 
        cmp cx,bx 
